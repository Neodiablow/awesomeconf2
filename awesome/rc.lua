--local myinclude = require("files.includes")
...
local mymenu = require("files.menu")
...
for s = 1, screen.count() do
	...
	vicious.register(netwidgeteth, vicious.widgets.net, '<span background="#535d6c" rise="2000"><span color="#ffffff" font="visitor TT2 BRK 12"> eth : <span font="visitor TT2 BRK 12" color="#7AC82E">${eth0 down_kb}</span> Kb <span font="Terminus 7">↓↑</span><span color="#ffffff"> <span font="visitor TT2 BRK 12" color="#46A8C3">${eth0 up_kb}</span> Kb </span></span></span>', 3)
...
globalkeys = awful.util.table.join(
    ...
    --{{Personal Key binding 
    	...
    	awful.key({ modkey, "Shift"   }, "!", function () os.execute("setxkbmap us -variant altgr-intl")   end),
    	awful.key({ modkey, "Shift"   }, "/", function () os.execute("setxkbmap fr")   end),
   
    	--lock screen
    	awful.key({ modkey, "Control"   }, "a", function () os.execute("i3lock -i /home/neodiablow/.config/awesome/themes/default/wallpaper/deafmute.png")   end),
...
--    awful.key({ modkey, "Shift"          }, "Return", function () c=awful.util.spawn(terminal) awful.client.setslave(c) end),
--    awful.key({ modkey, "Shift"          }, "Return", function () awful.client.getmaster() end),
        -- awful.client.setslave(c)
    awful.key({ modkey, "Control" }, "r", awesome.restart),
    awful.key({ modkey, "Shift"   }, "q", awesome.quit),

    --modification for make ranger the explorer and thunar as explorer2
    awful.key({modkey,		  }, "e", function() awful.util.spawn_with_shell("urxvt -e ranger") end),
    awful.key({modkey,		  }, "d", function() awful.util.spawn(explorer2) end),
...
--awful.util.spawn_with_shell("urxvtc -e wicd-curses");
